...
morgan.format(
  "myformat",
  '[:date[America/Toronto]] ":method :url" :status :res[content-length] - :response-time ms'
);
...
/* Default Endpoint */
app.get("/", (req, res) =>
  res.status(httpcodes.OK).render(`welcome.html`, {
    appName: packageJson.name,
    homepage: packageJson.homepage,
  })
);
app.get("/version", (req, res) => {
  res.status(httpcodes.OK).send(`Version ${packageJson.version}`);
});
    ...
    (error, results /* fields */) => {
      if (error) {
        res
          ...
          .send(`Could not perform query: ${error}`);
      } else {
...
app.get("*", (req, res) =>
  res.status(httpcodes.NOT_FOUND).render(`404.html`, { path: req.originalUrl })
);
